...
class PropertyRef {
  ...
  _parse(str, builder) {
    ...
    if (parts.length === 1) {
      ...
      const col = parts.pop();
      this.relation = (builder) => {
        const relationName = '[' + parts.join('.[') + ']'.repeat(parts.length);
        return builder.leftJoinRelation(relationName);
      };

      this.propertyName = parts.join(':') + '.' + col;
   ...
   */
  fullColumnName() {
    if (this.relation && typeof this.relation !== 'function' && this.relation.isOneToOne()) {
    ...
    } else if (typeof this.relation !== 'function') {
