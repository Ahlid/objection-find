...
class FindQueryBuilder {
  constructor(modelClass) {
    ...
    /**
     * Number of rows by each page.
     *
     * @type {Number}
     * @private
     */

    this._defaultRowsByPage = 25;
  ...
  build(params, builder) {
    ...
    this._buildPagination(params, builder);
    this._buildFinds(params, builder);
  ...
  _buildPagination(params, builder) {
    const countParam = _.find(params, { key: 'page' });
    if (countParam) {
      // console.log(countParam.value);
      const rowsByPage = _.find(params, { key: 'rowsByPage' });
      builder.page(countParam.value, rowsByPage ? rowsByPage.value || this._defaultRowsByPage : this._defaultRowsByPage);
    }
  }

  _buildFinds(params, builder) {
    let countParam = _.find(params, { key: 'findOne' });
    if (countParam) {
      return builder.findOne();
    }

    countParam = _.find(params, { key: 'findById' });
    if (countParam) {
      return builder.findById(countParam.value);
    }
  }
  ...
  _buildJoins(params, builder) {
    ...
    _.each(params, function(param) {
      _.each(param.propertyRefs, ref => {
        ...
        if (rel && typeof rel !== 'function' && rel.isOneToOne()) {
          relationsToJoin.push(rel);
        } else if (typeof rel === 'function') {
          relationsDeep.push(rel);
        }
      });
    });
    ...
    _.each(_.uniq(relationsDeep, 'name'), relation => {
      relation(builder);
    });

    if (!_.isEmpty(relationsToJoin) || !_.isEmpty(relationsDeep)) {
